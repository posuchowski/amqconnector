...
ok ( defined $M->{sip}   , 'Sip hashref is defined'    );
ok ( defined $M->{user}  , 'User hashref is defined'   );
ok ( defined $M->{device}, 'Device hashref is defined' );

diag( "Testing getters..." );
my $s = $M->get_sip();
my $u = $M->get_user();
my $d = $M->get_device();

ok ( defined $s, 'get_sip() seems to work' );
ok ( defined $u, 'get_user() seems to work' );
ok ( defined $d, 'get_device() seems to work' );

print "\n", colored( "Done testing, start dumping:", 'bold green on_black' ), "\n";
print STDERR "\n>>>> SIP: FINAL RESULT is a hashref:\n";
print STDERR Dumper $s, "\n";
print STDERR "<<<< END\n";

print STDERR "\n>>>> USERS: FINAL RESULT is a hashref:\n";
print STDERR Dumper $u, "\n";
print STDERR "<<<< END\n";

print STDERR "\n>>>> DEVICE: FINAL RESULT is a hashref:\n";
print STDERR Dumper $d, "\n";
print STDERR "<<<< END\n";

print colored( "FINISHED.", 'bold green on_black' ), "\n";
